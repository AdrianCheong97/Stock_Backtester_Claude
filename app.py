...
def chart_price(df: pd.DataFrame, result: dict, title: str) -> go.Figure:
    ...
    # Indicator overlays
    overlay_cfg = {
        "EMA8":   ("#58a6ff", 1),  "EMA21":  ("#d2a8ff", 1),  "EMA55":  ("#ffa657", 1),
        "EMA_F":  ("#58a6ff", 1.5),"EMA_S":  ("#ffa657", 1.5),
        "BB_M":   ("#388bfd", 1),  "DON_U":  ("#3fb95088", 1),"DON_L":  ("#f8514988", 1),
        "DON_M":  ("#d29922", 1),
    }
    ...
    for r in range(1, 4):
        fig.update_xaxes(row=r, col=1, **scroll_zoom_cfg)
        fig.update_yaxes(row=r, col=1, **scroll_zoom_cfg, fixedrange=False)
    return fig
